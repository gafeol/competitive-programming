...
public:
	...
	T qry(T x1, T y1, T x2, T y2) { // CUIDADO: Garantir que x2, y1-1; x1-1,y2; x1-1,y1-1; estao no pts do construtor
		return qry(x2, y2) - qry(x2, y1 - 1) - qry(x1 - 1, y2) + qry(x1 - 1, y1 - 1);
	}
 
	void upd(T x1, T y1, T x2, T y2, T v) { // CUIDADO: Garantir que x1,y2+1;x2+1,y1;x2+1,y2+1; estao no pts do construtor
		upd(x1, y1, v);
		upd(x1, y2 + 1, -v);
		upd(x2 + 1, y1, -v);
		upd(x2 + 1, y2 + 1, v);
